...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        cache = (x_norm, gamma, x_centered, inv_std, sample_var, eps)
        running_mean = momentum * running_mean + (1 - momentum) * sample_mean
        running_var = momentum * running_var + (1 - momentum) * sample_var
...
def batchnorm_backward(dout, cache):
    ...
    x_norm, gamma, x_centered, inv_std, sample_var, eps = cache
    N = dout.shape[0]
...
def conv_backward_naive(dout, cache):
    ...
    dx_padded = np.zeros((N, C, H + 2 * pad, W + 2 * pad), dtype=x.dtype)
    dw = np.zeros_like(w)
        ...
        for f in range(F):
            for i in range(H_out):
                for j in range(W_out):
                    ...
                    dx_padded[n, :, hs:hs + HH, ws:ws + WW] += w[f] * dout[n, f, i, j]

    dx = dx_padded[:, :, pad:pad + H, pad:pad + W] if pad else dx_padded
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    cache = (G, x_norm, x_group - mean, var, gamma, eps)
...
def spatial_groupnorm_backward(dout, cache):
    ...
    G, x_norm, x_centered, var, gamma, eps = cache
    N, C, H, W = dout.shape
    m = (C // G) * H * W
